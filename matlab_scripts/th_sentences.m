function [ th_vec, angle_vec ] = th_sentences( th_min, th_step, th_max, institution, window_size, speaker, position, method )
%[ th_vec, angle_vec ] = th_sentences( th_min, th_max, th_step, institution, window_size, speaker, position )
%   consider all sentences
...
for i = th_vec
    angle_vec(j) = allSentencesSolution(institution, window_size, th_vec(j), method, speaker, position); 
    j = j+1;
end
...
S_title = sprintf ('%s, N%d, speaker: %d, position: %d, method: %s', institution, window_size, speaker, position, method);
title(S_title);
