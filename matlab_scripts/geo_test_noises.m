function [ sol_pos_1, sol_pos_2, sol_pos_3, sol_pos_4 ] = geo_test_noises( th )
%% - GEO -
% all speaker(s)
% all sentence(s)
% 1 position(s)
...
%% Position 1
myFiles = dir(fullfile(myDir,'white_noise_p1_geo*')); %gets all txt files in struct
...
%% Position 2
myFiles = dir(fullfile(myDir,'white_noise_p2_geo*')); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
...
%% Position 3
myFiles = dir(fullfile(myDir,'white_noise_p3_geo*')); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
...
%% Position 4
myFiles = dir(fullfile(myDir,'white_noise_p4_geo*')); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
