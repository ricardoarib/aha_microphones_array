function [ sol_pos_1, sol_pos_2, sol_pos_3, sol_pos_4 ] = srp_one_speaker_all_sentences( th, speaker )
%% - srp -
% 1 speaker(s)
% all sentence(s)
% 1 position(s)
...
%% Position 1
file_name = strcat ('speaker-',num2str(speaker),'_*position-1_*.srp_results.txt');
...
%% Position 2
file_name = strcat ('speaker-',num2str(speaker),'_*position-2_*.srp_results.txt');
myFiles = dir(fullfile(myDir, file_name)); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
...
%% Position 3
file_name = strcat ('speaker-',num2str(speaker),'_*position-3_*.srp_results.txt');
myFiles = dir(fullfile(myDir, file_name)); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
...
%% Position 4
file_name = strcat ('speaker-',num2str(speaker),'_*position-4_*.srp_results.txt');
myFiles = dir(fullfile(myDir, file_name)); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
