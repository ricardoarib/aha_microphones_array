function [ sol_pos_1, sol_pos_2, sol_pos_3, sol_pos_4 ] = srp_all_speakers_all_sentences( th )
%% - srp -
% all speaker(s)
% all sentence(s)
% 1 position(s)
...
%% Position 1
myFiles = dir(fullfile(myDir,'speaker*position-1_*.srp_results.txt')); %gets all txt files in struct
...
%% Position 2
myFiles = dir(fullfile(myDir,'speaker*position-2_*.srp_results.txt')); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
...
%% Position 3
myFiles = dir(fullfile(myDir,'speaker*position-3_*.srp_results.txt')); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
...
%% Position 4
myFiles = dir(fullfile(myDir,'speaker*position-4_*.srp_results.txt')); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
