function [ sol_pos_1, sol_pos_2, sol_pos_3, sol_pos_4 ] = geo_all_speakers_one_sentence ( th, sentence )
%% - GEO -
% all speaker(s)
% 1 sentence(s)
% 1 position(s)
...
%% Position 1
file_name = strcat ('speaker*position-1_sentence-',num2str(sentence),'_*','.geo_results.txt');
...
%% Position 2
file_name = strcat ('speaker*position-2_sentence-',num2str(sentence),'_*','.geo_results.txt');
myFiles = dir(fullfile(myDir, file_name)); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
...
%% Position 3
file_name = strcat ('speaker*position-3_sentence-',num2str(sentence),'_*','.geo_results.txt');
myFiles = dir(fullfile(myDir, file_name)); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
...
%% Position 4
file_name = strcat ('speaker*position-4_sentence-',num2str(sentence),'_*','.geo_results.txt');
myFiles = dir(fullfile(myDir, file_name)); %gets all txt files in struct
num_files = size(myFiles, 1); % get number of files
